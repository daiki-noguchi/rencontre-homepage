...
// サーバーアクションを追加
import { getNewsById, updateNews } from "@/app/actions/news-actions"
...
export default function AdminNewsEditPage({
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        if (news.image_url) {
          // プレースホルダー画像かどうかをチェック
          if (news.image_url.includes("/placeholder.svg")) {
            console.warn("プレースホルダー画像が使用されています:", news.image_url)
            // プレースホルダー画像の場合は警告を表示
            toast({
              title: "注意",
              description: "画像がプレースホルダーになっています。新しい画像をアップロードしてください。",
              variant: "warning",
            })
          }
        ...
        console.error("ニュース記事の取得中にエラーが発生しました:", err)
        toast({
          title: "エラー",
          description: "ニュース記事の読み込みに失敗しました。",
          variant: "destructive",
        })
        router.push("/admin/news")
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("更新を開始します。ID:", id)
      console.log("フォームデータ:", formData)

      // サーバーアクションを使用してニュース記事を更新
      ...
      console.error("ニュース記事の更新中にエラーが発生しました:", error)

      // エラーメッセージをより詳細に
      let errorMessage = "ニュース記事の更新に失敗しました。もう一度お試しください。"
        ...
        <Card className="bg-white/90 backdrop-blur shadow-lg border-none overflow-hidden max-w-2xl mx-auto">
          <form onSubmit={handleSubmit}>
            <CardHeader>
              ...
              <CardDescription>ニュース記事の情報を編集してください。</CardDescription>
            </CardHeader>
